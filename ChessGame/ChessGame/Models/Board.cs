...
namespace ChessGame
{
    public class Board
    {
        ...
        */
        ...
        private void AddPawnsAndEmptyPieces()
        {
            for (int i = 0; i < 8; i++)
            {
                board[6, i] = new Pawn(P1); //White pawns
                board[1, i] = new Pawn(P2); //Black pawns
            }
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
